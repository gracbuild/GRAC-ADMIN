-- =====================================================================
-- 047 -- ACTIVATE action: re-enable an Inactive / Retired record
--
-- WHY THIS FILE EXISTS
-- --------------------
-- The ACTIVATE support spans one CHECK constraint and one procedure, both
-- of which live in 002_control_management_procedures.sql.  Re-running the
-- whole of 002 would work (it is guarded / CREATE OR ALTER throughout), but
-- 002 is ~3,700 lines and touches every object in the module.  This script
-- ships only the two things that changed.
--
-- WHAT IT DOES
-- ------------
--   1. Widens GRAC_New.change_management.ck_cm_chg_action to allow the
--      'Activate' action type alongside Add / Edit / Inactive.
--   2. Replaces dbo.cm_manage_repository with the definition that handles
--      the ACTIVATE action:
--        - @audit_action        -- ACTIVATE audits as N'Activate'
--        - maker-checker gate   -- @p_action IN ('SAVE','RETIRE','ACTIVATE')
--        - @change_action /
--          @change_payload      -- raises an 'Activate' change request
--                                  carrying {"status":"Active"}
--        - checker apply        -- action_type 'Activate' -> ACTIVATE
--        - auto-approve apply   -- same mapping on the self-approval path
--        - new ACTIVATE branch  -- sets status='Active' for every area the
--                                  RETIRE branch covers, with hierarchy
--                                  guards so a child cannot go Active under
--                                  an inactive parent (errors 50110-50121)
--
-- SYMPTOMS THIS FIXES
-- -------------------
--   "Authority Code is required." (THROW 50008)
--       The old procedure does not recognise ACTIVATE, so the request falls
--       through the dispatch chain into the entity SAVE branch and fails on
--       the empty payload.
--   "The INSERT statement conflicted with the CHECK constraint
--    ck_cm_chg_action"
--       The procedure now raises an 'Activate' change request, but the
--       constraint still only allowed Add / Edit / Inactive.
--
-- The constraint is re-added without WITH NOCHECK: existing rows only ever
-- hold Add / Edit / Inactive, so it validates cleanly.
--
-- Preflight: 002_control_management_procedures.sql must already be applied.
-- Rollback: database/047_repository_activate_action_rollback.sql
-- Safe to re-run.  ASCII-only.
-- =====================================================================

IF OBJECT_ID('dbo.cm_manage_repository','P') IS NULL
  THROW 51000, 'Apply 002_control_management_procedures.sql before this script.', 1;
GO
...
BEGIN
    ...
    PRINT '047: dropped the old ck_cm_chg_action.';
END
GO
...
BEGIN
    ...
    PRINT '047: ck_cm_chg_action now allows Activate.';
END
GO

-- ---------------------------------------------------------------------
-- 2. dbo.cm_manage_repository -- verbatim copy of the definition in 002
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Verification.  All three columns must read 1.
-- ---------------------------------------------------------------------
SELECT
  ...
  CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.cm_manage_repository')) LIKE '%''SAVE'',''RETIRE'',''ACTIVATE''%' THEN 1 ELSE 0 END AS HasMakerCheckerGate,
...
PRINT '047 applied: cm_manage_repository handles ACTIVATE and change_management accepts the Activate action type.';
GO
