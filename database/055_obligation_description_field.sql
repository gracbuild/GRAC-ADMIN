/* =====================================================================
   055_obligation_description_field.sql

   Obligation Description becomes a real, editable field on the Obligation
   Master screen, stored in requirement_obligation.obligation_text.

   THE PROBLEM
   -----------
   ObligationMasterForm collected Obligation Name, Type, Status, Keywords
   and Remarks. There was no description field, and obligation_text -- the
   column whose name says it holds one -- was written by
   cm_manage_repository as a verbatim copy of obligation_name:

       INSERT ... VALUES(@obm_obligation_name, @obm_obligation_name, ...)
       UPDATE ... SET obligation_text = COALESCE(@obm_obligation_name, obligation_text)

   So an Obligation could carry a description in the database, but no
   screen would show it and the first interactive Save would overwrite it
   with the name. The bulk loads had nowhere durable to put the wording
   from the source workbook.

   AFTER THIS SCRIPT
   -----------------
   The form's Remarks box is replaced by an Obligation Description box.
   It is editable, it is bound to obligation_text, and what a user types is
   what gets stored. Clearing the box clears the column.

   remarks keeps its column and its place in the payload contract, but the
   screen no longer collects it. It is a back-end field now: bulk loads and
   other server-side writers own it. The UPDATE branch COALESCEs it, so a
   save from the Obligation screen -- which sends no remarks key -- can no
   longer wipe a value written that way.

   WHAT IT SHIPS
   -------------
   Two procedures, re-emitted whole because T-SQL has no way to patch part
   of one. Both are otherwise byte-identical to the revisions they replace.

     1. dbo.cm_manage_repository  (from 050_practice_code_autogen.sql)
          * the 'obligations' save branch reads $.obligationText and writes
            it to obligation_text on INSERT and UPDATE;
          * that branch COALESCEs remarks instead of assigning it;
          * the audit field-label map renders 'obligationText' as
            'Obligation Description' rather than 'Obligation Name' -- four
            call sites, one per detail-row builder;
          * @record_reference for an obligation prefers obligation_name,
            falling back to the first 200 characters of obligation_text.
            It used to read obligation_text, which was the name; now that
            the column holds prose, an untrimmed reference would fill the
            Audit Trace grid with paragraphs.

     2. dbo.cm_manage_obligation_composite  (from
        046_obligation_composite_auto_approve.sql)
          * the master payload it hands to cm_manage_repository carries
            obligationText -- the dispatcher rebuilds that payload field by
            field rather than forwarding @p_payload, so a new member has to
            be named here or it never reaches the save branch;
          * the change_management before-snapshot carries obligationText
            too, so a maker-checker diff shows description edits.

   The read path needed nothing: cm_get_repository already returned both
   ObligationText and Remarks on the obligation detail queries.

   FRONT END
   ---------
   This script is one half of the change. The other half is in the web
   project and must ship with it:

     src/ControlManagement.Web/Views/Repository/ObligationMasterForm.cshtml
     src/ControlManagement.Web/wwwroot/js/obligation-master-form.js

   EXISTING ROWS
   -------------
   Nothing is back-filled. Every Obligation saved before this migration has
   obligation_text = obligation_name, so the new field shows the name until
   somebody edits it. Migration 054 writes real descriptions for the rows
   it loads. To see the split, run the verification queries at the end.

   Prerequisites : 050_practice_code_autogen.sql, 046_obligation_composite_auto_approve.sql
   Rerunnable    : yes -- CREATE OR ALTER, no data changes.
   Rollback      : 055_obligation_description_field_rollback.sql

   MAKER-CHECKER
   -------------
   Unchanged. 'obligations' stays is_maker_checker = 1 and interactive
   saves still route through change_management; only the columns the branch
   writes have moved.
   ===================================================================== */
...
BEGIN
    RAISERROR('055 preflight failed: dbo.cm_manage_repository is missing. Run 050 first.', 16, 1);
    SET NOEXEC ON;
END
GO
...
BEGIN
    RAISERROR('055 preflight failed: dbo.cm_manage_obligation_composite is missing. Run 046 first.', 16, 1);
    SET NOEXEC ON;
END
GO
...
BEGIN
    RAISERROR('055 preflight failed: requirement_obligation.obligation_text is missing. Run 001 first.', 16, 1);
    SET NOEXEC ON;
END
GO
...
--    Only the 'obligations' save branch, the audit label map and the
--    obligation @record_reference differ from 050.
-- ---------------------------------------------------------------------
...
GO
-- ---------------------------------------------------------------------
-- 2. dbo.cm_manage_obligation_composite
--    Only the two obligationText members differ from 046.
-- ---------------------------------------------------------------------
...
PRINT '055 complete. Obligation Description is now bound to obligation_text.';
PRINT 'Deploy the matching ObligationMasterForm.cshtml and obligation-master-form.js with this script.';
GO

-- ---------------------------------------------------------------------
-- Verification
-- ---------------------------------------------------------------------
-- The split between rows that carry a real description and rows still
-- holding the pre-055 copy of the name. Neither number is an error.
SELECT description_same_as_name =
         SUM(CASE WHEN obligation_text = obligation_name THEN 1 ELSE 0 END),
       description_present =
         SUM(CASE WHEN NULLIF(LTRIM(RTRIM(obligation_text)), N'') IS NOT NULL
                   AND obligation_text <> obligation_name THEN 1 ELSE 0 END),
       description_empty =
         SUM(CASE WHEN NULLIF(LTRIM(RTRIM(obligation_text)), N'') IS NULL THEN 1 ELSE 0 END),
       total = COUNT(*)
FROM GRAC_New.requirement_obligation;
GO

-- Both procedures should report today's date.
SELECT o.name, m.modify_date
FROM sys.objects o
JOIN sys.objects m ON m.object_id = o.object_id
WHERE o.name IN ('cm_manage_repository','cm_manage_obligation_composite')
ORDER BY o.name;
GO
