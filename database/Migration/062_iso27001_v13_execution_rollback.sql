/* =====================================================================
   062_iso27001_v13_execution_rollback.sql

   Reverses 062_iso27001_v13_execution.sql -- removes the 3587 Execution
   Obligations that load wrote, and everything hanging off them.

   HOW THE ROWS ARE IDENTIFIED
   ---------------------------
   By requirement_obligation.remarks, which 062 stamps with

       'Bulk load 062. ... sheet Execution ...'
...
IF @commit_rollback = 0
    PRINT N'062 rollback DRY RUN. Nothing will be deleted.';
...
WHERE ro.remarks LIKE N'Bulk load 062. %';

DECLARE @n INT = (SELECT COUNT(*) FROM #doomed);
PRINT CONCAT(N'  Execution Obligations matched by the 062 load stamp: ', @n, N' (loaded: 3587).');

DELETE l FROM GRAC_New.obligation_execution_evidence_link l JOIN #doomed d ON d.obligation_id = l.obligation_id;
DELETE e FROM GRAC_New.requirement_obligation_evidence e JOIN #doomed d ON d.obligation_id = e.obligation_id;
DELETE x FROM GRAC_New.obligation_execution_spec x JOIN #doomed d ON d.obligation_id = x.obligation_id;
...
PRINT CONCAT(N'  Deleted ', @@ROWCOUNT, N' Execution Obligation parent rows.');

IF @commit_rollback = 1
BEGIN
    COMMIT TRANSACTION;
    PRINT N'062 rollback COMMITTED.';
END
ELSE
BEGIN
    ROLLBACK TRANSACTION;
    PRINT N'062 rollback DRY RUN COMPLETE -- ROLLED BACK, nothing deleted. Set @commit_rollback = 1 to apply.';
END
...
SELECT COUNT(*) AS remaining_execution_obligations
FROM GRAC_New.requirement_obligation
WHERE remarks LIKE N'Bulk load 062. %';
GO
