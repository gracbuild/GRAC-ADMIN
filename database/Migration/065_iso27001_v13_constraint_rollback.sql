/* =====================================================================
   065_iso27001_v13_constraint_rollback.sql

   Reverses 065_iso27001_v13_constraint.sql -- removes the 1171 Constraint
   Obligations that load wrote, and everything hanging off them.

   HOW THE ROWS ARE IDENTIFIED
   ---------------------------
   By requirement_obligation.remarks, which 065 stamps with

       'Bulk load 065. ... sheet Constraint ...'
...
IF @commit_rollback = 0
    PRINT N'065 rollback DRY RUN. Nothing will be deleted.';
...
WHERE ro.remarks LIKE N'Bulk load 065. %';

DECLARE @n INT = (SELECT COUNT(*) FROM #doomed);
PRINT CONCAT(N'  Constraint Obligations matched by the 065 load stamp: ', @n, N' (loaded: 1171).');

DELETE l FROM GRAC_New.obligation_constraint_evidence_link l JOIN #doomed d ON d.obligation_id = l.obligation_id;
DELETE e FROM GRAC_New.requirement_obligation_evidence e JOIN #doomed d ON d.obligation_id = e.obligation_id;
DELETE x FROM GRAC_New.obligation_constraint_rule x JOIN #doomed d ON d.obligation_id = x.obligation_id;
...
PRINT CONCAT(N'  Deleted ', @@ROWCOUNT, N' Constraint Obligation parent rows.');

IF @commit_rollback = 1
BEGIN
    COMMIT TRANSACTION;
    PRINT N'065 rollback COMMITTED.';
END
ELSE
BEGIN
    ROLLBACK TRANSACTION;
    PRINT N'065 rollback DRY RUN COMPLETE -- ROLLED BACK, nothing deleted. Set @commit_rollback = 1 to apply.';
END
...
SELECT COUNT(*) AS remaining_constraint_obligations
FROM GRAC_New.requirement_obligation
WHERE remarks LIKE N'Bulk load 065. %';
GO
