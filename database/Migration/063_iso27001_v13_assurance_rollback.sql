/* =====================================================================
   063_iso27001_v13_assurance_rollback.sql

   Reverses 063_iso27001_v13_assurance.sql -- removes the 3044 Assurance
   Obligations that load wrote, and everything hanging off them.

   HOW THE ROWS ARE IDENTIFIED
   ---------------------------
   By requirement_obligation.remarks, which 063 stamps with

       'Bulk load 063. ... sheet Assurance ...'
...
IF @commit_rollback = 0
    PRINT N'063 rollback DRY RUN. Nothing will be deleted.';
...
WHERE ro.remarks LIKE N'Bulk load 063. %';

DECLARE @n INT = (SELECT COUNT(*) FROM #doomed);
PRINT CONCAT(N'  Assurance Obligations matched by the 063 load stamp: ', @n, N' (loaded: 3044).');

DELETE l FROM GRAC_New.obligation_assurance_evidence_link l JOIN #doomed d ON d.obligation_id = l.obligation_id;
DELETE e FROM GRAC_New.requirement_obligation_evidence e JOIN #doomed d ON d.obligation_id = e.obligation_id;
DELETE x FROM GRAC_New.obligation_assurance_spec x JOIN #doomed d ON d.obligation_id = x.obligation_id;
...
PRINT CONCAT(N'  Deleted ', @@ROWCOUNT, N' Assurance Obligation parent rows.');

IF @commit_rollback = 1
BEGIN
    COMMIT TRANSACTION;
    PRINT N'063 rollback COMMITTED.';
END
ELSE
BEGIN
    ROLLBACK TRANSACTION;
    PRINT N'063 rollback DRY RUN COMPLETE -- ROLLED BACK, nothing deleted. Set @commit_rollback = 1 to apply.';
END
...
SELECT COUNT(*) AS remaining_assurance_obligations
FROM GRAC_New.requirement_obligation
WHERE remarks LIKE N'Bulk load 063. %';
GO
