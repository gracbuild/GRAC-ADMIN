/* =====================================================================
   064_iso27001_v13_event_response_rollback.sql

   Reverses 064_iso27001_v13_event_response.sql -- removes the 1194 Event Response
   Obligations that load wrote, and everything hanging off them.

   HOW THE ROWS ARE IDENTIFIED
   ---------------------------
   By requirement_obligation.remarks, which 064 stamps with

       'Bulk load 064. ... sheet Event Response ...'
...
IF @commit_rollback = 0
    PRINT N'064 rollback DRY RUN. Nothing will be deleted.';
...
WHERE ro.remarks LIKE N'Bulk load 064. %';

DECLARE @n INT = (SELECT COUNT(*) FROM #doomed);
PRINT CONCAT(N'  Event Response Obligations matched by the 064 load stamp: ', @n, N' (loaded: 1194).');

DELETE l FROM GRAC_New.obligation_event_response_evidence_link l JOIN #doomed d ON d.obligation_id = l.obligation_id;
DELETE e FROM GRAC_New.requirement_obligation_evidence e JOIN #doomed d ON d.obligation_id = e.obligation_id;
DELETE x FROM GRAC_New.obligation_event_response x JOIN #doomed d ON d.obligation_id = x.obligation_id;
...
PRINT CONCAT(N'  Deleted ', @@ROWCOUNT, N' Event Response Obligation parent rows.');

IF @commit_rollback = 1
BEGIN
    COMMIT TRANSACTION;
    PRINT N'064 rollback COMMITTED.';
END
ELSE
BEGIN
    ROLLBACK TRANSACTION;
    PRINT N'064 rollback DRY RUN COMPLETE -- ROLLED BACK, nothing deleted. Set @commit_rollback = 1 to apply.';
END
...
SELECT COUNT(*) AS remaining_event_response_obligations
FROM GRAC_New.requirement_obligation
WHERE remarks LIKE N'Bulk load 064. %';
GO
