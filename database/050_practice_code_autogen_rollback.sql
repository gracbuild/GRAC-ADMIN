-- =====================================================================
-- 050 ROLLBACK -- restore hand-entered Practice Code
--
-- Puts dbo.cm_manage_repository back to its 049 definition: the
-- 'requirements' save branch writes whatever Code the payload carries and
-- generates nothing.
--
-- IMPORTANT
-- ---------
-- Practices created while 050 was active keep their generated PR-### codes.
-- This script only stops NEW codes from being generated -- it does not undo
-- codes already issued, and it must not, because those codes are referenced
-- by mappings, change requests and the audit trail.
--
-- After running this, restore Practice Code as a required field on the Add
-- Practice form (repository.js schemas + formSchema) and re-add "code" to
-- RepositoryCommandValidator, or every new Practice save will fail on the
-- NOT NULL requirement_code column.
--
-- Preflight: 002, then 047, 048, 049, 050.
...
IF EXISTS(SELECT 1 FROM sys.sql_modules WHERE object_id=OBJECT_ID('dbo.cm_manage_repository') AND definition LIKE '%@practice_code%')
  THROW 51050, 'Rollback failed: the Practice Code generator is still present.', 1;
SELECT 'cm_manage_repository' AS ObjectName, 'reverted to 049 definition' AS Result;
GO
