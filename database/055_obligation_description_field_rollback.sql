/* =====================================================================
   055_obligation_description_field_rollback.sql

   Reverses 055_obligation_description_field.sql by re-emitting both
   procedures exactly as they shipped:

     dbo.cm_manage_repository            from 050_practice_code_autogen.sql
     dbo.cm_manage_obligation_composite  from 046_obligation_composite_auto_approve.sql

   After this script the 'obligations' save branch once again writes
   obligation_text = obligation_name on INSERT and UPDATE, assigns remarks
   unconditionally, renders 'obligationText' as 'Obligation Name' in the
   audit field-label map, and builds @record_reference from obligation_text.
   The composite dispatcher stops forwarding obligationText.

   NO DATA IS CHANGED. Descriptions typed while 055 was live stay in
   obligation_text and remain legible in the database, but the first
   interactive Save of an Obligation after this rollback overwrites that
   row's description with its name -- that is the 050 behaviour being
   restored, not a defect. Export anything worth keeping first:

       SELECT obligation_id, obligation_name, obligation_text
       FROM GRAC_New.requirement_obligation
       WHERE obligation_text <> obligation_name;

   REVERT THE FRONT END TOO. ObligationMasterForm.cshtml and
   obligation-master-form.js must go back to their pre-055 revisions in the
   same deployment. Left as they are, the form posts $.obligationText to a
   procedure that ignores it and collects no remarks for a procedure that
   expects them, so description edits appear to save and silently do
   nothing.

   Rerunnable : yes -- CREATE OR ALTER.
   ===================================================================== */
...
BEGIN
    RAISERROR('055 rollback preflight failed: dbo.cm_manage_repository is missing.', 16, 1);
    SET NOEXEC ON;
END
GO

-- ---------------------------------------------------------------------
-- 1. dbo.cm_manage_repository, restored to the 050 revision verbatim.
-- ---------------------------------------------------------------------
...
GO
-- ---------------------------------------------------------------------
-- 2. dbo.cm_manage_obligation_composite, restored to the 046 revision
--    verbatim.
-- ---------------------------------------------------------------------
...
PRINT '055 rollback complete. Both procedures restored to their pre-055 revisions.';
PRINT 'Revert ObligationMasterForm.cshtml and obligation-master-form.js to match.';
GO
