-- =====================================================================
-- 048 -- Cascade deactivation and restore
--
-- WHAT THIS CHANGES
-- -----------------
-- Until now, deactivating a parent left its whole subtree live: an Inactive
-- Authority still had Active Artifacts, Releases, Source Structure nodes,
-- Source Classifications and Framework Statements underneath it, all still
-- selectable in forms and mappings.  Only Obligations cascaded (to their own
-- evidence types).
--
-- After this script:
--
--   Inactive  -- the subtree goes down with the parent, and every row the
--                cascade touched is recorded against a batch id
--   Activate  -- that batch is restored: exactly the rows the cascade took
--                down, each back to the status it held beforehand
--
-- Rows that were ALREADY inactive when the cascade ran are neither touched nor
-- recorded, so re-activating a parent never revives something the user had
...
-- descendants here and are unaffected.
--
-- WHAT IT SHIPS
-- -------------
--   1. GRAC_New.cm_cascade_deactivation      -- the batch record
--   2. dbo.fn_cm_repository_descendants      -- the subtree of a record
--   3. dbo.fn_cm_repository_descendant_status-- the same, with current status
--   4. dbo.cm_manage_repository              -- cascade on RETIRE, restore on
--                                               ACTIVATE, and the read-only
--                                               RETIRE_IMPACT preview used by
--                                               the confirmation dialog
--
-- Preflight: 002 and 047 (this supersedes 047's copy of the procedure; running
--            047 first is harmless, running it AFTER this would regress the
--            cascade).
...
IF OBJECT_ID('dbo.cm_manage_repository','P') IS NULL
  THROW 51001, 'Apply 002_control_management_procedures.sql before this script.', 1;
GO
...
BEGIN
    ALTER TABLE GRAC_New.change_management
        ADD CONSTRAINT ck_cm_chg_action
            CHECK (action_type IN (N'Add', N'Edit', N'Inactive', N'Activate'));
END
GO

-- ---------------------------------------------------------------------
-- 1. Cascade batch record
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. dbo.cm_manage_repository -- verbatim copy of the definition in 002
-- ---------------------------------------------------------------------
...
BEGIN
 ...
 BEGIN
  ...
  BEGIN
    ...
    DECLARE @change_payload NVARCHAR(MAX)=CASE WHEN @p_action='RETIRE' THEN JSON_MODIFY(N'{}','$.status',N'Inactive') WHEN @p_action='ACTIVATE' THEN JSON_MODIFY(N'{}','$.status',N'Active') ELSE @p_payload END;
 ...
 BEGIN
   ...
   BEGIN
     ...
     BEGIN
       ...
       UPDATE t SET status='Inactive',updated_by=@p_usr_id,updated_dt=SYSUTCDATETIME()
...
-- ---------------------------------------------------------------------
-- Verification.  Every column must read 1.
-- ---------------------------------------------------------------------
SELECT
  CASE WHEN OBJECT_ID('GRAC_New.cm_cascade_deactivation','U') IS NOT NULL                                       THEN 1 ELSE 0 END AS HasCascadeTable,
  CASE WHEN OBJECT_ID('dbo.fn_cm_repository_descendants','IF') IS NOT NULL                                      THEN 1 ELSE 0 END AS HasDescendantsFn,
  CASE WHEN OBJECT_ID('dbo.fn_cm_repository_descendant_status','IF') IS NOT NULL                                THEN 1 ELSE 0 END AS HasDescendantStatusFn,
  CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.cm_manage_repository')) LIKE '%@p_action=''ACTIVATE''%'            THEN 1 ELSE 0 END AS HasActivateBranch,
  CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.cm_manage_repository')) LIKE '%RETIRE_IMPACT%'                     THEN 1 ELSE 0 END AS HasImpactPreview,
  CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.cm_manage_repository')) LIKE '%cm_cascade_deactivation%'           THEN 1 ELSE 0 END AS HasCascadeWiring,
  CASE WHEN EXISTS(SELECT 1 FROM sys.check_constraints
                   WHERE name = 'ck_cm_chg_action'
                     AND parent_object_id = OBJECT_ID('GRAC_New.change_management')
                     AND definition LIKE '%Activate%')                                                         THEN 1 ELSE 0 END AS ActionCheckAllowsActivate;
GO

PRINT '048 applied: deactivation now cascades, and activation restores the cascaded batch.';
GO
